...
We provide both binary packages and instructions for how to compile from source for all 3 platforms (see `install instructions </Get-Started/Installation>` as well as `documentation <https://docs.ros2.org/beta2/>`__).
...
* Typesupport has been redesigned so that you only build a single executable and can choose one of the available RMW implementations by setting an environment variable (see `documentation </Get-Started/Installation/RMW-Implementations/Working-with-multiple-RMW-implementations>`).
...
* A set of command-line tools using the extensible ``ros2`` command (see `conceptual article </Developer-Tools/Introspection-and-analysis/About-Command-Line-Tools>`).
...
* `Dummy_robot demo </Capabilities/Simulation/dummy-robot/dummy-robot-demo>`:
